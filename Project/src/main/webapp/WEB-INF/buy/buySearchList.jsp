...
$(document).ready(function(){
	...
	function goWrite(){
		if(${mvo.login_id==null}){
			...
			location.href='${cpath}/buyboardWriteForm.do';
		}
	}
        ...
        <div class="page-body">
          <div class="container-xl">
            ...
            <div class="bg-light border-y">
                ...
                <table class="table table-vcenter table-hover">
                  ...
                  <tbody id="list">
                  	<c:forEach  var="vo" items="${list2 }"> 
    					<tr>
			    			<td class ="text-center">${vo.buy_seq }</td>
			    			<td><a href="${cpath }/buyboardView.do?num=${vo.buy_seq}&p=${board2.currentPage}&login_id=${mvo.login_id}" class="text-reset">${vo.buy_title }<span class="text-orange">&nbsp;[${vo.buy_cmtcnt }]</span></a></td>
			    			<td class="text-muted text-center"><img src="#" alt="icon">${vo.login_id }</td>
			    			<td class="text-muted text-center">${vo.buy_regdate }</td>
			    			<td class="text-muted  text-center">${vo.buy_cnt }</td>
			    		</tr>
    				</c:forEach>
              ...
              <div class="card-footer d-flex align-items-center py-2">
                ...
                <c:if test="${board2.pagingPerCount<board2.start_page}">
                  <li class="page-item abled">
                    <a class="page-link" href="${cpath }/buySearch.do?p=${board2.start_page-1}&val=${val}&text=${text}" tabindex="-1" aria-abled="true">
				    ...
				    <c:forEach var="i" begin="${board2.start_page}" end="${board2.end_page}">
				    	...
				    	<li class="page-item active"><a class="page-link" href="${cpath }/buylist.do?p=${i}">${i}</a></li>
				    	</c:when>
				    	<c:otherwise>
					    <li class="page-item"><a class="page-link" href="${cpath }/buySearch.do?p=${i}&val=${val}&text=${text}">${i}</a></li>
					    </c:otherwise>
					    </c:choose>
				    </c:forEach>
				...
				<c:if test="${board2.numPageCount>board2.end_page}">
					<li class="page-item abled"><a class="page-link" href="${cpath }/buySearch.do?p=${board2.end_page+1}&val=${val}&text=${text}" tabindex="1" aria-abled="true">next
					<svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" 
                      stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" 
                      stroke-linejoin="round">
              ...
              <form action="${cpath }/buySearch.do" method="get" id="searchForm" onsubmit="return checksearch()">
